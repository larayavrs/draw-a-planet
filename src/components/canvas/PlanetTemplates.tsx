...
// Each template is a pure function that draws onto a Fabric.js canvas.
// They return a Promise<void> so async fabric imports are handled uniformly.
...
type TemplateId = "rock" | "ice" | "lava" | "ocean" | "desert" | "gas";

interface Template {
  id: TemplateId;
  label: string;
  emoji: string;
  /** Preview background gradient for the thumbnail */
  previewGradient: string;
  draw: (fc: FabricCanvas, fabric: FabricLib) => void;
}

// Minimal subset of Fabric types we use in templates
type FabricLib = {
...
const TEMPLATES: Template[] = [
  {
    ...
    draw(fc, { Circle }) {
      ...
      for (let i = 0; i < 120; i++) {
        ...
        fc.add(new Circle({
          radius: r, left: x - r, top: y - r,
          fill: c, opacity: rand() * 0.5 + 0.2,
      ...
      for (let i = 0; i < 20; i++) {
        ...
        fc.add(new Circle({
          radius: r, left: x - r, top: y - r,
          fill: "transparent",
          stroke: "#2a1e10", strokeWidth: rand() * 3 + 1,
  ...
  },
  {
    ...
    draw(fc, { Circle, Rect }) {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        fc.add(new Rect({
          left: x, top: y,
          width: rand() * 80 + 10, height: rand() * 3 + 1,
          angle: rand() * 360,
          fill: "#7ab8e0",
      ...
      // Polar caps (top and bottom)
      fc.add(new Circle({
        radius: 200, left: CANVAS_SIZE / 2 - 200, top: -140,
        fill: "#eef8ff", opacity: 0.85,
        selectable: false, evented: false,
      }));
      fc.add(new Circle({
        radius: 180, left: CANVAS_SIZE / 2 - 180, top: CANVAS_SIZE - 60,
        fill: "#eef8ff", opacity: 0.75,
        selectable: false, evented: false,
      }));
      ...
      for (let i = 0; i < 30; i++) {
        ...
        fc.add(new Circle({
          radius: r,
          left: rand() * CANVAS_SIZE - r,
          top: rand() * CANVAS_SIZE - r,
          fill: "#f0faff", opacity: rand() * 0.5 + 0.2,
          selectable: false, evented: false,
        }));
      }
      fc.requestRenderAll();
  ...
  },
  {
    ...
    draw(fc, { Circle, Path }) {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        fc.add(new Circle({
          radius: r, left: x - r, top: y - r,
          fill: poolColors[Math.floor(rand() * poolColors.length)],
          opacity: rand() * 0.6 + 0.2,
          selectable: false, evented: false,
        }));
      }
      // Lava cracks — jagged paths
      for (let i = 0; i < 25; i++) {
        const sx = rand() * CANVAS_SIZE;
        const sy = rand() * CANVAS_SIZE;
        let path = `M ${sx} ${sy}`;
        for (let j = 0; j < 4; j++) {
          path += ` L ${sx + (rand() - 0.5) * 120} ${sy + (rand() - 0.5) * 120}`;
        }
        fc.add(new Path(path, {
          stroke: rand() > 0.5 ? "#ff8030" : "#ffb060",
          strokeWidth: rand() * 4 + 1,
          fill: "transparent",
      ...
      for (let i = 0; i < 20; i++) {
        ...
        fc.add(new Circle({
          ...
          left: rand() * CANVAS_SIZE - r,
          top: rand() * CANVAS_SIZE - r,
          fill: "#1a0800", opacity: rand() * 0.7 + 0.3,
          selectable: false, evented: false,
        }));
      }
      fc.requestRenderAll();
  ...
  },
  {
    ...
    draw(fc, { Rect, Circle }) {
      ...
      for (let i = 0; i < layerColors.length; i++) {
        fc.add(new Rect({
          left: 0, top: (CANVAS_SIZE / layerColors.length) * i,
          width: CANVAS_SIZE, height: CANVAS_SIZE / layerColors.length + 2,
          fill: layerColors[i], opacity: 0.7,
          selectable: false, evented: false,
        }));
      }
      // Bright streaks / light caustics
      for (let i = 0; i < 35; i++) {
        fc.add(new Rect({
          left: rand() * CANVAS_SIZE,
          top: rand() * CANVAS_SIZE,
          width: rand() * 100 + 20, height: rand() * 4 + 1,
          angle: rand() * 40 - 20,
          fill: "#60d0ff", opacity: rand() * 0.25 + 0.05,
          selectable: false, evented: false,
        }));
      }
      // Continents / islands
      for (let i = 0; i < 5; i++) {
        ...
        fc.add(new Circle({
          ...
          left: rand() * CANVAS_SIZE - r,
          top: rand() * CANVAS_SIZE - r,
          fill: "#1a8040", opacity: rand() * 0.5 + 0.3,
          selectable: false, evented: false,
        }));
      }
      // Foam highlights at top
      for (let i = 0; i < 20; i++) {
        const r = rand() * 15 + 3;
        fc.add(new Circle({
          radius: r,
          left: rand() * CANVAS_SIZE - r,
          top: rand() * 80 - r,
          fill: "#a0e8ff", opacity: rand() * 0.3 + 0.1,
          selectable: false, evented: false,
        }));
      }
      fc.requestRenderAll();
  ...
  },
  {
    ...
    draw(fc, { Rect, Circle }) {
      ...
      // Dune band stripes
      const duneColors = [
        "#d08040", "#c87030", "#e09050", "#b86028",
        "#d89060", "#c07030", "#e8a070",
      ];
      for (let i = 0; i < duneColors.length; i++) {
        fc.add(new Rect({
          left: 0, top: (CANVAS_SIZE / duneColors.length) * i,
          width: CANVAS_SIZE, height: CANVAS_SIZE / duneColors.length + 2,
          fill: duneColors[i], opacity: 0.75,
          selectable: false, evented: false,
        }));
      }
      // Dune ridges (ellipses)
      for (let i = 0; i < 18; i++) {
        fc.add(new Rect({
          left: rand() * CANVAS_SIZE,
          top: rand() * CANVAS_SIZE,
          width: rand() * 180 + 60, height: rand() * 12 + 3,
          angle: rand() * 15 - 7,
          fill: "#f0c080",
          rx: 30, ry: 30,
          opacity: rand() * 0.4 + 0.1,
      ...
      for (let i = 0; i < 8; i++) {
        const r = rand() * 30 + 8;
        fc.add(new Circle({
          radius: r, left: rand() * CANVAS_SIZE - r, top: rand() * CANVAS_SIZE - r,
          fill: "transparent", stroke: "#904020", strokeWidth: 2,
  ...
  },
  {
    ...
    draw(fc, { Rect, Circle }) {
      ...
      for (const band of bands) {
        ...
        fc.add(new Rect({
          left: 0, top: y, width: CANVAS_SIZE, height: h + 2,
          fill: band.color, opacity: 0.85,
          selectable: false, evented: false,
        }));
        y += h;
      }
      // Storm eye (Jupiter-style)
      fc.add(new Circle({
        radius: 40, left: CANVAS_SIZE * 0.65 - 40, top: CANVAS_SIZE * 0.45 - 40,
        fill: "#d04820", opacity: 0.8,
        selectable: false, evented: false,
      }));
      fc.add(new Circle({
        radius: 25, left: CANVAS_SIZE * 0.65 - 25, top: CANVAS_SIZE * 0.45 - 25,
        fill: "#e86030", opacity: 0.9,
        selectable: false, evented: false,
      }));
      // Small eddies
      for (let i = 0; i < 8; i++) {
...
export function PlanetTemplates({ tier, locale, activeTemplate, onSelect }: PlanetTemplatesProps) {
  ...
  const applyTemplate = useCallback(
    async (template: Template) => {
      ...
      const fc = fabricRef.current as FabricCanvas;
      const { Circle, Rect, Line, Path } = await import("fabric");
      const fabricLib: FabricLib = {
        Circle: Circle as unknown as FabricLib["Circle"],
        Rect: Rect as unknown as FabricLib["Rect"],
        Line: Line as unknown as FabricLib["Line"],
        Path: Path as unknown as FabricLib["Path"],
      };
      template.draw(fc, fabricLib);
